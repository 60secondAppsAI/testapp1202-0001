					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateStorage()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Storages = true">Add</base-button>
								              <modal :show.sync="modal2Storages">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Storage Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="StorageId" type="text" placeholder="Enter StorageId" v-model="storageDetails.storageId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Storage Location</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="StorageLocation" type="text" placeholder="Enter StorageLocation" v-model="storageDetails.storageLocation"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateStorage() {
      ...
      const jsonData = JSON.stringify(this.storageDetails);
      const res = await StorageService.update(jsonData);
...
//        this.getAllStorages();
    ...
    async getStorageDetails() {
      try {
			let response = await StorageService.get(this.storageId);
