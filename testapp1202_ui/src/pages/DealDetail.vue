					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateDeal()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Deals = true">Add</base-button>
								              <modal :show.sync="modal2Deals">
								  ...
								  <base-input label="SubmissionDate" type="text" placeholder="Enter SubmissionDate" v-model="dealToAdd.submissionDate"></base-input>
								  <base-input label="Status" type="text" placeholder="Enter Status" v-model="dealToAdd.status"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Deal Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="DealId" type="text" placeholder="Enter DealId" v-model="dealDetails.dealId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Submission Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="dealDetails.submissionDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Status</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Status" type="text" placeholder="Enter Status" v-model="dealDetails.status"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateDeal() {
      ...
      const jsonData = JSON.stringify(this.dealDetails);
      const res = await DealService.update(jsonData);
...
//        this.getAllDeals();
    ...
    async getDealDetails() {
      try {
			let response = await DealService.get(this.dealId);
