					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSmsNotification()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2SmsNotifications = true">Add</base-button>
								              <modal :show.sync="modal2SmsNotifications">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Sms Notification Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SmsNotificationId" type="text" placeholder="Enter SmsNotificationId" v-model="smsNotificationDetails.smsNotificationId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Message</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Message" type="text" placeholder="Enter Message" v-model="smsNotificationDetails.message"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      ...
      modal2SmsNotifications: false,
      smsNotificationDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2SmsNotifications = false;
    ...
    async updateSmsNotification() {
      ...
      const jsonData = JSON.stringify(this.smsNotificationDetails);
      const res = await SmsNotificationService.update(jsonData);
...
//        this.getAllSmsNotifications();
    ...
    async getSmsNotificationDetails() {
      try {
			let response = await SmsNotificationService.get(this.smsNotificationId);
			this.smsNotificationDetails = response.data;
