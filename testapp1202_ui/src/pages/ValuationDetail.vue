					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateValuation()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Valuations = true">Add</base-button>
								              <modal :show.sync="modal2Valuations">
								  ...
								  <base-input label="ValuationRange" type="text" placeholder="Enter ValuationRange" v-model="valuationToAdd.valuationRange"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Valuation Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ValuationId" type="text" placeholder="Enter ValuationId" v-model="valuationDetails.valuationId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Valuation Range</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ValuationRange" type="text" placeholder="Enter ValuationRange" v-model="valuationDetails.valuationRange"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateValuation() {
      ...
      const jsonData = JSON.stringify(this.valuationDetails);
      const res = await ValuationService.update(jsonData);
...
//        this.getAllValuations();
    ...
    async getValuationDetails() {
      try {
			let response = await ValuationService.get(this.valuationId);
			this.valuationDetails = response.data;
