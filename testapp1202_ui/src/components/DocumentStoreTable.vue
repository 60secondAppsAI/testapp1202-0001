        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Document Stores</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDocumentStores = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDocumentStores">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add DocumentStore</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DocumentStoreId" type="text" placeholder="Enter DocumentStoreId" v-model="documentStoreToAdd.documentStoreId"></base-input>
  <base-input label="DocumentPath" type="text" placeholder="Enter DocumentPath" v-model="documentStoreToAdd.documentPath"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="documentStores" :row-key="record => record.DocumentStoreId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DocumentStorePictureView :documentStores="documentStores" />
          </div>
...
import DocumentStorePictureView from './DocumentStorePictureView.vue';


const documentStoresColumns = [
  "documentStoreId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    DocumentStorePictureView
  },

  data() {
    return {
      modalDocumentStores: false,
        isTableView: true,

      columns: [
        {
          title: 'Document Store Id',
		dataIndex: 'documentStoreId',
          visible: true,
          scopedSlots: { customRender: 'documentStoreId' },
          sorter: true
          //sorter: (a, b) => a.documentStoreId - b.documentStoreId,
          //sorter: (a, b) => a.documentStoreId.localeCompare(b.documentStoreId),
        },
        {
          title: 'Document Path',
		dataIndex: 'documentPath',
          ...
          sorter: true
          //sorter: (a, b) => a.documentPath - b.documentPath,
          //sorter: (a, b) => a.documentPath.localeCompare(b.documentPath),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} documentStores`,
      },
      ...
      documentStoresTable: {
        columns: [...documentStoresColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'documentStoreId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDocumentStoresTable() {
      ...
      let documentStoresTableData = [];
      for (let i = 0; i < this.documentStores.length; i++) {
        documentStoresTableData.push({
          id: i,
          documentStoreId: this.documentStores[i].documentStoreId,
          year: this.documentStores[i].year,
          date: this.documentStores[i].date,
          competitionId: this.documentStores[i].competitionId,
          documentStoreId: this.documentStores[i].documentStoreId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-document-stores',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDocumentStoresChanged', this.searchQuery);
		//this.renderDocumentStoresTable();
    },
	...
	async handleAddSubmitted() {
      this.modalDocumentStores = false;

      const currentDate = new Date().getTime();
      this.documentStoreToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.documentStoreToAdd);
      console.log(jsonData);
      
      const res = await DocumentStoreService.addDocumentStore(jsonData);
