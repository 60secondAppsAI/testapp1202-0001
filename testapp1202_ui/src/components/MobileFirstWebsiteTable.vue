        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Mobile First Websites</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalMobileFirstWebsites = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMobileFirstWebsites">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MobileFirstWebsite</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MobileFirstWebsiteId" type="text" placeholder="Enter MobileFirstWebsiteId" v-model="mobileFirstWebsiteToAdd.mobileFirstWebsiteId"></base-input>
  <base-input label="WebsiteUrl" type="text" placeholder="Enter WebsiteUrl" v-model="mobileFirstWebsiteToAdd.websiteUrl"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="mobileFirstWebsites" :row-key="record => record.MobileFirstWebsiteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MobileFirstWebsitePictureView :mobileFirstWebsites="mobileFirstWebsites" />
          </div>
...
import MobileFirstWebsitePictureView from './MobileFirstWebsitePictureView.vue';


const mobileFirstWebsitesColumns = [
  "mobileFirstWebsiteId",
  "year",
  "date",
  "competitionId",
  "mobileFirstWebsiteId"
...
export default {
  ...
  components: {
    ...
    Pagination,
    MobileFirstWebsitePictureView
  },

  data() {
    return {
      modalMobileFirstWebsites: false,
        isTableView: true,

      columns: [
        {
          title: 'Mobile First Website Id',
		dataIndex: 'mobileFirstWebsiteId',
          visible: true,
          scopedSlots: { customRender: 'mobileFirstWebsiteId' },
          sorter: true
          //sorter: (a, b) => a.mobileFirstWebsiteId - b.mobileFirstWebsiteId,
          //sorter: (a, b) => a.mobileFirstWebsiteId.localeCompare(b.mobileFirstWebsiteId),
        },
        {
          title: 'Website Url',
		dataIndex: 'websiteUrl',
          ...
          sorter: true
          //sorter: (a, b) => a.websiteUrl - b.websiteUrl,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} mobileFirstWebsites`,
      },

      mobileFirstWebsites: [],
      mobileFirstWebsiteToAdd: {},

      mobileFirstWebsitesTable: {
        columns: [...mobileFirstWebsitesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'mobileFirstWebsiteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMobileFirstWebsitesTable() {
      ...
      let mobileFirstWebsitesTableData = [];
      for (let i = 0; i < this.mobileFirstWebsites.length; i++) {
        mobileFirstWebsitesTableData.push({
          id: i,
          mobileFirstWebsiteId: this.mobileFirstWebsites[i].mobileFirstWebsiteId,
          year: this.mobileFirstWebsites[i].year,
          date: this.mobileFirstWebsites[i].date,
          competitionId: this.mobileFirstWebsites[i].competitionId,
          mobileFirstWebsiteId: this.mobileFirstWebsites[i].mobileFirstWebsiteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-mobile-first-websites',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMobileFirstWebsitesChanged', this.searchQuery);
		//this.renderMobileFirstWebsitesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMobileFirstWebsites = false;

      const currentDate = new Date().getTime();
      this.mobileFirstWebsiteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.mobileFirstWebsiteToAdd);
      console.log(jsonData);
      
      const res = await MobileFirstWebsiteService.addMobileFirstWebsite(jsonData);
