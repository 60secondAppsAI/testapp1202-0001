        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Deal Dashboards</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDealDashboards = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDealDashboards">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add DealDashboard</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DealDashboardId" type="text" placeholder="Enter DealDashboardId" v-model="dealDashboardToAdd.dealDashboardId"></base-input>
  <base-input label="RepAssigned" type="text" placeholder="Enter RepAssigned" v-model="dealDashboardToAdd.repAssigned"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="dealDashboards" :row-key="record => record.DealDashboardId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DealDashboardPictureView :dealDashboards="dealDashboards" />
          </div>
...
import DealDashboardPictureView from './DealDashboardPictureView.vue';


const dealDashboardsColumns = [
  "dealDashboardId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    DealDashboardPictureView
  },

  data() {
    return {
      modalDealDashboards: false,
        isTableView: true,

      columns: [
        {
          title: 'Deal Dashboard Id',
		dataIndex: 'dealDashboardId',
          visible: true,
          scopedSlots: { customRender: 'dealDashboardId' },
          sorter: true
          //sorter: (a, b) => a.dealDashboardId - b.dealDashboardId,
          //sorter: (a, b) => a.dealDashboardId.localeCompare(b.dealDashboardId),
        },
        {
          title: 'Rep Assigned',
		dataIndex: 'repAssigned',
          visible: true,
          scopedSlots: { customRender: 'repAssigned' },
          sorter: true
          //sorter: (a, b) => a.repAssigned - b.repAssigned,
          //sorter: (a, b) => a.repAssigned.localeCompare(b.repAssigned),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} dealDashboards`,
      },
      ...
      dealDashboardsTable: {
        columns: [...dealDashboardsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'dealDashboardId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDealDashboardsTable() {
      ...
      let dealDashboardsTableData = [];
      for (let i = 0; i < this.dealDashboards.length; i++) {
        dealDashboardsTableData.push({
          id: i,
          dealDashboardId: this.dealDashboards[i].dealDashboardId,
          year: this.dealDashboards[i].year,
          date: this.dealDashboards[i].date,
          competitionId: this.dealDashboards[i].competitionId,
          dealDashboardId: this.dealDashboards[i].dealDashboardId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-deal-dashboards',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDealDashboardsChanged', this.searchQuery);
		//this.renderDealDashboardsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalDealDashboards = false;

      const currentDate = new Date().getTime();
      this.dealDashboardToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.dealDashboardToAdd);
      console.log(jsonData);
      
      const res = await DealDashboardService.addDealDashboard(jsonData);
