        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalEmailNotifications = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalEmailNotifications">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add EmailNotification</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="EmailNotificationId" type="text" placeholder="Enter EmailNotificationId" v-model="emailNotificationToAdd.emailNotificationId"></base-input>
  <base-input label="Subject" type="text" placeholder="Enter Subject" v-model="emailNotificationToAdd.subject"></base-input>
  <base-input label="Body" type="text" placeholder="Enter Body" v-model="emailNotificationToAdd.body"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="emailNotifications" :row-key="record => record.EmailNotificationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <EmailNotificationPictureView :emailNotifications="emailNotifications" />
          </div>
...
import EmailNotificationPictureView from './EmailNotificationPictureView.vue';


const emailNotificationsColumns = [
  "emailNotificationId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    EmailNotificationPictureView
  },

  data() {
    return {
      modalEmailNotifications: false,
        isTableView: true,

      columns: [
        {
          title: 'Email Notification Id',
		dataIndex: 'emailNotificationId',
          visible: true,
          scopedSlots: { customRender: 'emailNotificationId' },
          sorter: true
          //sorter: (a, b) => a.emailNotificationId - b.emailNotificationId,
          //sorter: (a, b) => a.emailNotificationId.localeCompare(b.emailNotificationId),
        },
        {
          title: 'Subject',
		dataIndex: 'subject',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} emailNotifications`,
      },

      emailNotifications: [],
      emailNotificationToAdd: {},

      emailNotificationsTable: {
        columns: [...emailNotificationsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'emailNotificationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderEmailNotificationsTable() {
      ...
      let emailNotificationsTableData = [];
      for (let i = 0; i < this.emailNotifications.length; i++) {
        emailNotificationsTableData.push({
          id: i,
          emailNotificationId: this.emailNotifications[i].emailNotificationId,
          year: this.emailNotifications[i].year,
          date: this.emailNotifications[i].date,
          competitionId: this.emailNotifications[i].competitionId,
          emailNotificationId: this.emailNotifications[i].emailNotificationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-email-notifications',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForEmailNotificationsChanged', this.searchQuery);
		//this.renderEmailNotificationsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalEmailNotifications = false;

      const currentDate = new Date().getTime();
      this.emailNotificationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.emailNotificationToAdd);
      console.log(jsonData);
      
      const res = await EmailNotificationService.addEmailNotification(jsonData);
