        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Seo Strategys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalSeoStrategys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSeoStrategys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add SeoStrategy</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="SeoStrategyId" type="text" placeholder="Enter SeoStrategyId" v-model="seoStrategyToAdd.seoStrategyId"></base-input>
  <base-input label="StrategyDescription" type="text" placeholder="Enter StrategyDescription" v-model="seoStrategyToAdd.strategyDescription"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="seoStrategys" :row-key="record => record.SeoStrategyId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SeoStrategyPictureView :seoStrategys="seoStrategys" />
          </div>
...
import SeoStrategyPictureView from './SeoStrategyPictureView.vue';


const seoStrategysColumns = [
  "seoStrategyId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    SeoStrategyPictureView
  },

  data() {
    return {
      modalSeoStrategys: false,
        isTableView: true,

      columns: [
        {
          title: 'Seo Strategy Id',
		dataIndex: 'seoStrategyId',
          visible: true,
          scopedSlots: { customRender: 'seoStrategyId' },
          sorter: true
          //sorter: (a, b) => a.seoStrategyId - b.seoStrategyId,
          //sorter: (a, b) => a.seoStrategyId.localeCompare(b.seoStrategyId),
        },
        {
          title: 'Strategy Description',
		dataIndex: 'strategyDescription',
          visible: true,
          scopedSlots: { customRender: 'strategyDescription' },
          sorter: true
          //sorter: (a, b) => a.strategyDescription - b.strategyDescription,
          //sorter: (a, b) => a.strategyDescription.localeCompare(b.strategyDescription),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} seoStrategys`,
      },
      ...
      seoStrategysTable: {
        columns: [...seoStrategysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'seoStrategyId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSeoStrategysTable() {
      ...
      let seoStrategysTableData = [];
      for (let i = 0; i < this.seoStrategys.length; i++) {
        seoStrategysTableData.push({
          id: i,
          seoStrategyId: this.seoStrategys[i].seoStrategyId,
          year: this.seoStrategys[i].year,
          date: this.seoStrategys[i].date,
          competitionId: this.seoStrategys[i].competitionId,
          seoStrategyId: this.seoStrategys[i].seoStrategyId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-seo-strategys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSeoStrategysChanged', this.searchQuery);
		//this.renderSeoStrategysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalSeoStrategys = false;

      const currentDate = new Date().getTime();
      this.seoStrategyToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.seoStrategyToAdd);
      console.log(jsonData);
      
      const res = await SeoStrategyService.addSeoStrategy(jsonData);
